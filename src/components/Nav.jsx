...
const Nav = () => {
  ...
  return (
    <nav className={`z-[9999999] fixed top-0 w-full bg-white`}>
        ...
        {(username && navigate && !location.includes('/addblog')) && <div className='x_xl:flex xs:hidden items-center justify-start w-9/12 mx-auto my-3 flex-wrap max-h-[90px] overflow-y-scroll'>
            <h1 className='text-xl font-bold mr-3'>Related Tags :</h1>
            {
                loading ? <PuffLoader color={color} loading={loading} size={18} aria-label="Loading Spinner" data-testid="loader"/> : TagsAll.map((tag,idx) => (
                    <Link onClick={() => setShow(!show)} key={idx} to={`/relatedBlogs/${tag._id}`} className='drop flex items-center my-1 justify-center rounded-[10px] border-solid p-1 border-blue-800 border-[1px] mx-1 w-auto cursor-pointer group scale-100 transition'><p className='text-xs font-normal'>{tag.tag}</p> <BsLink45Deg className={'group-hover:scale-110 group-hover:text-red-500 drop text-xl'} /></Link>
                ))
            }
        ...
        <div className={`${show ? 'right-0' : 'right-[-110%]'} x_xl:hidden xs:flex pt-16  bg-white z-[999] top-0 max-h-screen h-screen items-start justify-start flex-col w-full absolute`}>
            ...
            <div className='buttons flex items-start justify-start z-[999] w-11/12 flex-1 mx-auto my-3 max-h-[100px] flex-wrap overflow-y-scroll'>
               ...
               {
                loading ? <PuffLoader color={color} loading={loading} size={18} aria-label="Loading Spinner" data-testid="loader"/> : TagsAll.map((tag,idx) => (
                    <Link onClick={() => setShow(!show)} key={idx} to={`/relatedBlogs/${tag._id}`} className='drop flex items-center my-1 justify-center rounded-[10px] border-solid p-1 border-blue-800 border-[1px] mx-1 w-auto cursor-pointer group scale-100 transition'><p className='text-xs font-normal'>{tag.tag}</p> <BsLink45Deg className={'group-hover:scale-110 text-xl group-hover:text-red-500 drop'} /></Link>
                ))
            }
            </div>}
