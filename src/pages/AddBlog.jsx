...
const AddBlog = () => {
  ...
  let AddPost = async () => {
    ...
    try {
        ...
        let UploadPost = await Axios.post('/blog',{
          title : title,
          desc : value,
          tag : filters,
          images : ImagesCloud,
  ...
  return (
    <div className='xs:w-11/12 xs:pt-20 md:pt-0 md:w-9/12 mx-auto  md:py-3 relative'>
      ...
      <h1 className='text-2xl my-2'>Body</h1>
      <ReactQuill className='' theme="snow" value={value} onChange={setValue} />
      <h1 className='text-2xl my-2'>Tags</h1>
       <Multiselect onSelect={(e) => setTags(e)} isObject={false} options={TagsAll.map((data) => (data.tag + ' ' +  `(${data._id})`))} />
      <h1 className='text-2xl my-2'>Upload Image</h1>
       <label htmlFor='image' className='cursor-pointer p-2 border-solid border-[1px] border-black/40 my-3 w-full'>
          <input multiple onChange={(e) => {handleReload(); setImages(e.target.files);}} type="file" name="image" id="image" className='hidden' />
          <FcPicture classNametext-4xl/> 
       </label>
        {images.length > 0 && <abbr title='use this to remove All the images'><li onClick={handleReload} className='cursor-pointer list-none absolute top-[420px] right-3 w-10 h-10 flex items-center justify-center rounded-full bg-black/20'><AiOutlineReload /></li></abbr>}
      ...
      <button onClick={() => AddPost()} className='pointer-events-auto p-2 my-3 text-sm rounded-sm cursor-pointer bg-black text-white w-full'>{loading ? <PuffLoader color={color} loading={loading} size={18} aria-label="Loading Spinner" data-testid="loader"/> : 'Add Post'}</button>
    </div>
  )
}
